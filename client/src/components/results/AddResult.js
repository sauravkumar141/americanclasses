...
import {
  StyledTableCell,
  StyledTableRow,
} from "../../assets/jss/material-kit-react/components/tableStyle";
...
function AddNotifications(props) {
  ...
  const handleUpload = useCallback(async () => {
    setLoading(true);
    results.sort((obj1, obj2) => obj2.marks - obj1.marks);
    var rank = 1;
    let arr = results.map((obj1, index) => {
      if (index > 0 && results[index].marks < results[index - 1].marks) {
        rank++;
      }
    ...
    let res = await axios.post("/api/saveResult", data);
    ...
    setTimeout(() => {
      res.data
        ...
        : pushMessageToSnackbar({
            text: "Your result has not been saved..",
          });
      fetchRandomResults();
      onClose();
    }, 2000);
  ...
  const addResult = useCallback(() => {
    if (name && parseInt(marks) <= parseInt(total)) {
      setResults([...results, { name, marks }]);
    }
    setName("");
    setMarks("");
    nameRef.current.focus();
  }, [setResults, setName, setMarks, nameRef, name, results, marks, total]);

  const openDeleteResult = useCallback(
    (id) => {
      const _results = [...results];
      const index = _results.findIndex(
        (element, index) => index === results.length - (id + 1)
      );
        ...
        >
          ...
          >
            ...
            <Box mb={2}>
              ...
              <Grid spacing={0} container>
                <h4>{`Total Students: ${studentdata.length}`}</h4>
              </Grid>
...
AddNotifications.propTypes = {
  ...
  fetchRandomResults: PropTypes.isRequired,
};
