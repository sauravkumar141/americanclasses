...
function AddTestSeries(props) {
  ...
  const openDeleteQuestion= useCallback(() =>{
    const _testSeries= [...testSeries];
    const index = _testSeries.findIndex((element, index) => index === (testSeries.length - (selectedID+1)));
          _testSeries.splice(index, 1);
          setTestSeries(_testSeries);
          handleClose();
  },[testSeries, selectedID, setTestSeries, handleClose]);
  ...
  const handleUpload = useCallback(async() => {
    ...
    let res= await axios.post('/api/saveTestSeries', data);
  
    setTimeout(() => {
      res.data ? (
        pushMessageToSnackbar({text: "Your testSeries has saved.."})
      ) : (
        pushMessageToSnackbar({text: "Your testSeries has not saved.."})
      )
      fetchTestSeries();
      onClose();
    }, 3000);
   
  }, [setLoading, fetchTestSeries, subject, pushMessageToSnackbar, onClose, testSeries, selectedFile, desp, state, choosedDate]);
  ...
  const addTestSeries= useCallback(()=>{
    if(question && a && b && c && d && correctAnswer){
        ...
        id++;
        if(mode === "edit"){
         // testSeries[selectedID]=obj;
         testSeries[testSeries.length - (selectedID+1)]=obj;

        }else{
          setTestSeries([...testSeries, obj]);
        }
    }
  setQuestion('');
  setA('');
  ...
  },[nameRef, question, a, b, c, d, correctAnswer, testSeries, setTestSeries, setQuestion, setA, setB, setC, setD,
   setCorrectAnswer, mode, setMode, selectedID]);
  ...
  const handleQuestion = useCallback(
    ...
    const handleTextDescription = useCallback(
      ...
      const handleDate = useCallback(
       ...
       },[handleClose, openDeleteQuestion, openEditQuestion, open, selectedID, testSeriesData])
          ...
          <Fragment>
            <Box pt={1}>
             <Paper style={{ maxWidth: theme.breakpoints.values[maxWidth], backgroundColor: "#bcbbd4" }}>
                 ...
                 <Typography paragraph variant="h5" style={{color: "rebeccapurple"}}>Upload TestSeries</Typography>
                 <Box mb={2}>
...
AddTestSeries.propTypes = {
  ...
  fetchTestSeries: PropTypes.isRequired
};

export default withStyles(styles, { withTheme: true })(AddTestSeries);
